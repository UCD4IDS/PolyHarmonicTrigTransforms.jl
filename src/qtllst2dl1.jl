module QTLLST2DL1
    ...
    function recurs_testdst2d(signal::AbstractVecOrMat, level::Int, lmax::Int)
        ...
        r = dst(dst(r)')' .* (sqrt(4 / (m - 1) / (n - 1)))
        cost = norm(r[:], 1) #Normalisation since matlab dst is not unitary

        r = nothing #We won't need it anymore and it's better to free the memory...
        lap = nothing
        ...
        eb1 = b1[2:m2-1] .- (b1[1] .+ [1:m2-2;]' ./ (m2 - 1) .* (b1[m2] - b1[1]))
        eb1 = dst(eb1) .* sqrt(2 / (m2 - 1))

        eb2 = b1[m2+1:m-1] .- (b1[m2] .+ [1:m2-2;]' ./ (m2 - 1) .* (b1[m] - b1[m2]))
        eb2 = dst(eb2) .* sqrt(2 / (m2 - 1))

        eb3 = b2[2:n2-1] .- (b2[1] .+ [1:n2-2;] ./ (n2 - 1) .* (b2[n2] - b2[1]))
        eb3 = dst(eb3) .* sqrt(2 / (n2 - 1))

        eb4 = b2[n2+1:n-1] .- (b2[n2] .+ [1:n2-2;] ./ (n2 - 1) .* (b2[n] - b2[n2]))
